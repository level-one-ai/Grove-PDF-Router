...
/**
 * Clean up a name field that may have an email address pasted into it.
 * Some Cin7 records have an email accidentally entered into firstName or lastName.
 * If the field contains an '@', strip out the email-looking part and return
 * whatever remains. If nothing remains, return empty string so the caller
 * can fall back to other sources.
 */
function sanitiseCin7NameField(value) {
  if (!value) return '';
  const trimmed = String(value).trim();
  if (!trimmed.includes('@')) return trimmed;

  // Strip email-looking tokens (anything containing @)
...
function buildResult(contact, order, searchName, pdfRef, matchMethod, fileId, pageNumber) {
  // Company name from contact takes priority — this is the master record
  const cin7Company  = (contact.company || '').trim() || null;

  // Sanitise name fields: some Cin7 records have emails entered into firstName.
  // Strip any '@'-containing tokens before constructing the customer name.
  const rawFirst = (contact.firstName || '').trim();
  const rawLast  = (contact.lastName  || '').trim();
  const cleanFirst = sanitiseCin7NameField(rawFirst);
  const cleanLast  = sanitiseCin7NameField(rawLast);
  let cin7Customer = `${cleanFirst} ${cleanLast}`.trim() || null;

  // If EITHER raw name field contained an '@', the Cin7 record has polluted
  // data. Prefer the PDF-extracted searchName since it came from clean text.
  // Only override if searchName itself doesn't contain '@'.
  const cin7HasEmailPollution = rawFirst.includes('@') || rawLast.includes('@');
  if (cin7HasEmailPollution && searchName && !searchName.includes('@')) {
    cin7Customer = searchName;
    console.log(`[cin7] Detected email-pollution in Cin7 name fields — using PDF name "${searchName}" instead`);
  }

  // Final safety: if result is somehow still polluted or empty, fall back to searchName
  if (!cin7Customer || cin7Customer.length < 2 || cin7Customer.includes('@')) {
    cin7Customer = (searchName && !searchName.includes('@')) ? searchName : null;
  }
  ...
  // Cache in Firestore — per page, not per file. A multi-page PDF has
  // different customers per page, so we must NOT share the cache between pages.
  if (fileId && pageNumber) {
    try {
      const db = require('./firebase');
      db.updateRecord(fileId, { [`cin7Lookup_${pageNumber}`]: result }).catch(() => {});
    } catch (e) {
 ...
 * @param {number} params.pageNumber    - Page number (for per-page caching)
 * @returns {object|null}
 */
async function lookupCin7FolderName({ customerName, companyName, pdfRef, pdfPostcode, pdfMobile, fileId, pageNumber }) {
  const auth = cin7Auth();
  if (!auth) {
  ...
  // Check Firestore cache — per page key
  if (fileId && pageNumber) {
    try {
      const db = require('./firebase');
      const record = await db.getRecord(fileId);
      const cached = record?.[`cin7Lookup_${pageNumber}`];
      if (cached) {
        console.log(`[cin7] Using cached result for ${fileId} page ${pageNumber}`);
        return cached;
      }
    } catch (e) {
  ...
  return buildResult(contact, matchedOrder, searchName, pdfRef, matchMethod, fileId, pageNumber);
}
