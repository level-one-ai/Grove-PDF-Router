...
async function findAllMatchingFolders(name, parentId, isCompany = false) {
  ...
  if (isCompany) {
    ...
    // Return exact matches first, fall back to two-word matches
    ...
    console.log(`[googleDrive] No match for company "${name}" — will create new folder`);
    return [];

  } else {
    // Customer name — exact match only after stripping titles
    ...
    if (exactMatches.length > 0) {
      ...
      console.log(`[googleDrive] No match for customer "${name}" — will create new folder`);
    }
    return exactMatches;
  }
}
