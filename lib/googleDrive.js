...
// Cached OAuth client — reused across calls within the same serverless invocation
 ...
 * when the access token expires (every hour) it uses the refresh token
 * to get a new one without any manual intervention.
 *
 * We also listen for token updates so that if Google rotates the refresh
 * token we save the new one to Vercel automatically, preventing
 * future invalid_grant errors.
 */
function getOAuthClient() {
  ...
  if (!clientId || !clientSecret || !refreshToken) {
    throw new Error('Missing Google OAuth credentials — check GOOGLE_OAUTH_CLIENT_ID, GOOGLE_OAUTH_CLIENT_SECRET, GOOGLE_OAUTH_REFRESH_TOKEN in Vercel env vars');
  }

  const client = new google.auth.OAuth2(clientId, clientSecret);
  client.setCredentials({ refresh_token: refreshToken });

  // Listen for token refreshes — if Google issues a new refresh token, save it
  client.on('tokens', async (tokens) => {
    if (tokens.access_token) {
      console.log('[googleDrive] Access token refreshed automatically');
    }
    if (tokens.refresh_token) {
      console.log('[googleDrive] Google issued a new refresh token — saving to Vercel');
      try {
        process.env.GOOGLE_OAUTH_REFRESH_TOKEN = tokens.refresh_token;
        _oauthClient = null; // reset so next invocation picks up new token
        ...
        if (vercelToken && projectId) {
          ...
          const req = https.request({
            ...
            path:     `/v10/projects/${projectId}/env${teamQs}`,
            method:   'POST',
            headers:  {
            ...
            console.log(`[googleDrive] Vercel env update: ${res.statusCode}`);
          });
          req.on('error', e => console.warn('[googleDrive] Vercel env update failed (non-fatal):', e.message));
          req.write(body);
          req.end();
        } else {
          console.warn('[googleDrive] VERCEL_TOKEN/VERCEL_PROJECT_ID not set — new refresh token saved to memory only');
        }
      } catch (e) {
        console.warn('[googleDrive] Token save error (non-fatal):', e.message);
      }
    }
  });
...
function getDriveClient() {
  // Prefer OAuth (personal account — avoids service account storage quota issues)
  if (
...
async function findAllMatchingFolders(name, parentId, isCompany = false) {
  ...
  // Paginate through ALL folders — Drive caps each page at 1000.
  // Without pagination, customers whose folder sits beyond page 1
  // (>1000 folders) would never be matched and a duplicate would be created.
  const folders = [];
  let pageToken = null;
  let pagesScanned = 0;
  do {
    ...
    pagesScanned++;
    if (pagesScanned > 50) {
      // Safety cap — 50 pages × 1000 = 50,000 folders, well beyond any realistic case
      console.warn(`[googleDrive] Pagination safety cap hit at ${folders.length} folders`);
      break;
    }
  } while (pageToken);

  console.log(`[googleDrive] Found ${folders.length} folders across ${pagesScanned} page(s) in parent ${parentId}`);
