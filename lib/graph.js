...
async function uploadFile(folderPath, fileName, fileBuffer) {
  ...
  // Request webUrl in $select so the file's SharePoint browser URL is returned
  const url = `https://graph.microsoft.com/v1.0/users/${userId}/drive/root:/${cleanPath}/${encodeURIComponent(fileName)}:/content?$select=id,name,webUrl`;
...
async function createSubscription(notificationUrl) {
  ...
  return await graphRequest('POST', '/subscriptions', {
    changeType: 'updated', // OneDrive for Business only supports 'updated' — new files also fire as 'updated' on the parent folder
