...
async function createSubscription(notificationUrl) {
  ...
  // Expiry: 3 days (maximum allowed for OneDrive)
  const expiry = new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString();

  // Resource path must use the items/{id} format for personal OneDrive
  // First get the folder item ID
  const folderPath = 'Grove Group Scotland/Grove Bedding/Scans';
  const folderInfo = await graphRequest(
    'GET',
    `/users/${userId}/drive/root:/${folderPath}`
  );
  ...
  return await graphRequest('POST', '/subscriptions', {
    ...
    resource: `/users/${userId}/drive/items/${folderId}`,
