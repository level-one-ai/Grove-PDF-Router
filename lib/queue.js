...
async function dispatchPageToMake(buffer, pageNumber, zeroPadded, fileId, originalFileName, totalPages) {
  ...
  // Convert buffer to hex string — this is what Make.com/Claude sees
  // as the "data" field (e.g. "25 50 44 46 2d 31...")
  const hexData = buffer.toString('hex').match(/.{1,2}/g).join(' ');

  // Send as JSON so every field is a direct named mappable field in Make.com.
  // No arrays, no headers — just flat fields.
  const payload = {
    fileName,
    fileId,
    pageNumber,
