 ...
 * Metadata sent as query parameters so Make.com exposes them
 * as individual named fields rather than a headers array.
 */
async function dispatchPageToMake(buffer, pageNumber, zeroPadded, fileId, originalFileName, totalPages) {
  const webhookUrl = process.env.MAKE_WEBHOOK_URL;

  // Build query string — Make.com shows these as individual named fields
  const params = new URLSearchParams({
    fileId,
  ...
  await axios.post(urlWithParams, buffer, {
    headers: {
      'Content-Type': 'application/pdf',
    },
    maxBodyLength: Infinity,
